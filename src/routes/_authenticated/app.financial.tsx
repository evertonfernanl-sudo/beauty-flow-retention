...
import { ArrowDown, ArrowUp, DollarSign, Plus, Search, Wallet, AlertTriangle, Target, Heart, Download } from "lucide-react";
...
const EXPENSE_CATEGORIES = [
  "Aluguel", "Internet", "Energia", "Água", "Produtos",
  "Marketing", "Impostos", "Funcionários", "Outros",
];
...
function FinancialPage() {
  ...
  const monthSummary = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        .from("financial_transactions").select("type, amount, transaction_date")
        .eq("company_id", companyId!).gte("transaction_date", start.toISOString().slice(0, 10));
      const rows = data ?? [];
  ...
  const recoverable = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("recovery_dashboard").select("potential_revenue, recovered_value_month")
  ...
  const goalPct = goal > 0 ? Math.min(100, Math.round((monthSummary.data?.income ?? 0) / goal * 100)) : 0;

  function exportCSV() {
    const rows = filtered.map((r: any) =>
      [r.transaction_date, r.type, r.category, (r.description ?? "").replace(/[,;]/g, " "), r.payment_method ?? "", r.amount].join(","));
    const csv = ["data,tipo,categoria,descricao,pagamento,valor", ...rows].join("\n");
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = `financeiro_${range.from}_${range.to}.csv`; a.click();
    ...
    <div className="space-y-6 pb-24">
      <header className="grid grid-cols-[minmax(0,1fr)_auto] items-end gap-3">
        <div className="min-w-0">
          ...
          <p className="text-sm text-muted-foreground">Quanto entrou, quanto saiu e quanto sobrou.</p>
        </div>
        <NewTransactionDialog
          open={open} onOpenChange={setOpen} companyId={companyId}
          onSaved={() => {
      ...
      <section className="grid gap-3 grid-cols-2 lg:grid-cols-4">
        <Kpi label="Receitas (mês)"        value={formatBRL(monthSummary.data?.income ?? 0)} tone="success"  icon={ArrowUp} />
        <Kpi label="Despesas (mês)"        value={formatBRL(monthSummary.data?.expense ?? 0)} tone="destructive" icon={ArrowDown} />
        <Kpi label="Lucro (mês)"           value={formatBRL(monthSummary.data?.profit ?? 0)} tone="primary" icon={DollarSign} highlight />
        <Kpi label="Receita Recuperável"   value={formatBRL(Number(recoverable.data?.potential_revenue ?? 0))} tone="primary" icon={Heart} />
      </section>
        ...
        {goal > 0 ? (
          <>
            <div className="flex items-baseline justify-between mb-2">
              <p className="text-2xl font-semibold tabular-nums">{formatBRL(monthSummary.data?.income ?? 0)}</p>
              <p className="text-sm text-muted-foreground">de {formatBRL(goal)} <span className="font-semibold text-primary">· {goalPct}%</span></p>
            </div>
            <Progress value={goalPct} />
          </>
        ) : (
          <p className="text-sm text-muted-foreground">Defina uma meta para acompanhar o progresso do mês.</p>
        )}
      </Card>
        ...
        <div className="h-56">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={flow30.data ?? []}>
              ...
              <Area type="monotone" dataKey="Entradas" stroke="hsl(var(--success))" fill="url(#g-in)" />
              <Area type="monotone" dataKey="Saídas"   stroke="hsl(var(--destructive))" fill="url(#g-out)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </Card>
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap items-center gap-3">
          <Tabs value={period} onValueChange={(v) => setPeriod(v as Period)} className="flex-1 min-w-[260px]">
            <TabsList>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <div className="relative flex-1 min-w-[200px]">
            ...
            <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar descrição, categoria, valor…" className="pl-9" />
          </div>
        </div>
      </div>
      ...
      <GoalDialog
        open={goalOpen} onOpenChange={setGoalOpen}
        companyId={companyId} current={goal}
...
function Kpi({
  label, value, tone, icon: Icon, highlight,
}: { label: string; value: string; tone: "success" | "destructive" | "primary"; icon: any; highlight?: boolean }) {
  const color = tone === "success" ? "text-success" : tone === "destructive" ? "text-destructive" : "text-primary";
  const bg = tone === "success" ? "bg-success/15 text-success" : tone === "destructive" ? "bg-destructive/15 text-destructive" : "gradient-primary text-primary-foreground";
  return (
...
function NewTransactionDialog({
...
}: { open: boolean; onOpenChange: (v: boolean) => void; companyId?: string; onSaved: () => void }) {
  ...
  async function onSubmit(v: z.infer<typeof newSchema>) {
    ...
    const { error } = await supabase.from("financial_transactions").insert({
      ...v, description: v.description || null, payment_method: v.payment_method || null,
      company_id: companyId,
    });
    if (error) return toast.error(error.message);
    toast.success(v.type === "INCOME" ? "Receita registrada" : "Despesa registrada");
    form.reset({
      type: v.type, category: "", description: "", amount: 0,
      payment_method: "", transaction_date: v.transaction_date,
    });
    onOpenChange(false);
    onSaved();
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogTrigger asChild>
        <Button className="shrink-0"><Plus className="h-4 w-4 mr-1" /> Lançamento</Button>
      </DialogTrigger>
      <DialogContent>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2"><Label>Data</Label><Input type="date" {...form.register("transaction_date")} /></div>
            <div className="space-y-2">
...
function GoalDialog({
  open, onOpenChange, companyId, current, onSaved,
}: { open: boolean; onOpenChange: (v: boolean) => void; companyId?: string; current: number; onSaved: () => void }) {
  const [value, setValue] = useState(String(current || ""));
  async function save() {
    if (!companyId) return;
    const num = Number(value);
    if (Number.isNaN(num) || num < 0) return toast.error("Valor inválido");
    const { error } = await supabase.from("companies").update({ monthly_revenue_goal: num }).eq("id", companyId);
    if (error) return toast.error(error.message);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader><DialogTitle>Meta de faturamento mensal</DialogTitle></DialogHeader>
        <div className="space-y-2">
          <Label>Meta (R$)</Label>
          <Input type="number" step="0.01" value={value} onChange={(e) => setValue(e.target.value)} placeholder="Ex: 10000" />
        </div>
        <DialogFooter>
