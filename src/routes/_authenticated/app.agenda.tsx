...
import {
  Plus, Calendar as CalendarIcon, Check, ChevronLeft, ChevronRight, X, Edit3, Search,
} from "lucide-react";
...
function AgendaPage() {
  ...
  const clients = useQuery({
    ...
    queryFn: async () => (await supabase.from("clients").select("id, name").eq("company_id", companyId!).order("name")).data ?? [],
  });
  const services = useQuery({
    enabled: !!companyId,
    queryKey: ["services-options", companyId],
    queryFn: async () => (await supabase.from("services").select("id, name, price, duration_minutes").eq("company_id", companyId!).eq("active", true).order("name")).data ?? [],
  });
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild><Button><Plus className="h-4 w-4 mr-1" /> Agendar</Button></DialogTrigger>
            <DialogContent>
              ...
              <form onSubmit={form.handleSubmit(onCreate)} className="space-y-3">
                ...
                <FormSelectField
                  ...
                  options={(services.data ?? []).map((s: any) => ({
                    value: s.id, label: `${s.name} · ${formatBRL(Number(s.price))}`,
                  }))}
                />
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-2"><Label>Data</Label><Input type="date" {...form.register("date")} /></div>
                  <div className="space-y-2"><Label>Hora</Label><Input type="time" {...form.register("time")} /></div>
                </div>
                ...
                <DialogFooter><Button type="submit" disabled={form.formState.isSubmitting}>Agendar</Button></DialogFooter>
              </form>
            </DialogContent>
          </Dialog>
        </div>
      ...
      <Card className="p-4 space-y-4">
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <Input placeholder="Buscar cliente, telefone ou serviço..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
          </div>
        ...
        {list.isLoading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground">Aproveite para cadastrar o próximo atendimento.</p>
          </div>
        ) : (
...
function AppointmentRow({ a, onComplete, onConfirm, onChanged, services }: {
  ...
  return (
    <li className="py-3 flex items-center justify-between gap-3">
      <Link to="/app/clients/$clientId" params={{ clientId: a.client_id }} className="flex items-center gap-3 min-w-0 flex-1 hover:opacity-80">
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="icon" variant="ghost" title="Editar / Reagendar"><Edit3 className="h-4 w-4" /></Button>
      </DialogTrigger>
      <DialogContent>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2"><Label>Valor (R$)</Label><Input type="number" step="0.01" value={price} onChange={(e) => setPrice(e.target.value)} /></div>
        </div>
...
function CancelAppointment({ a, onChanged }: { a: any; onChanged: () => void }) {
  ...
  async function save() {
    const { error } = await supabase.from("appointments").update({ status: type, cancellation_reason: reason || null }).eq("id", a.id);
    if (error) { toast.error(error.message); return; }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="icon" variant="ghost" title="Cancelar"><X className="h-4 w-4" /></Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader><DialogTitle>Cancelar atendimento</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <div className="space-y-2">
            <Label>Motivo</Label>
          ...
          <div className="space-y-2">
            ...
            <Textarea rows={3} placeholder="Por que foi cancelado?" value={reason} onChange={(e) => setReason(e.target.value)} />
          </div>
        </div>
        <DialogFooter>
          <Button variant="destructive" onClick={save}>Confirmar</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
function FormSelectField({ label, name, form, options }: { label: string; name: any; form: any; options: { value: string; label: string }[] }) {
  return (
    <div className="space-y-2">
      <Label>{label}</Label>
      <Controller
...
function rangeFor(view: ViewMode, cursor: Date): { start: Date; end: Date; label: string } {
  ...
  if (view === "day") {
    ...
    return { start, end, label: d.toLocaleDateString("pt-BR", { weekday: "long", day: "2-digit", month: "long", year: "numeric" }) };
  }
  if (view === "week") {
    const start = new Date(d); start.setDate(start.getDate() - start.getDay()); start.setHours(0, 0, 0, 0);
    const end = new Date(start); end.setDate(end.getDate() + 7);
    return { start, end, label: `Semana de ${start.toLocaleDateString("pt-BR")} a ${new Date(end.getTime() - 86400000).toLocaleDateString("pt-BR")}` };
  }
...
function statusLabel(s: string) {
  return ({ SCHEDULED: "Agendado", CONFIRMED: "Confirmado", COMPLETED: "Concluído", CANCELLED: "Cancelado", NO_SHOW: "Faltou" } as Record<string, string>)[s] ?? s;
}
