...
import {
  AlertCircle, ArrowDownRight, ArrowUpRight, Calendar, DollarSign,
  RefreshCw, Receipt, TrendingUp, Users,
} from "lucide-react";
    ...
    queryFn: async () => {
      ...
      const [tx, appts, clientsAll, returnsLate, services] = await Promise.all([
        ...
        supabase.from("clients")
          .select("id, status")
          .eq("company_id", companyId!),
        supabase.from("return_opportunities")
          .select("id", { count: "exact", head: true })
          .eq("company_id", companyId!)
          .eq("converted", false)
          .in("status", ["DUE", "LATE"]),
        supabase.from("service_metrics")
      ...
      let revenue = 0, expense = 0, revenuePrev = 0, expensePrev = 0;
      for (const r of tx.data ?? []) {
        const d = r.transaction_date;
        const amt = Number(r.amount);
        if (d >= startKey) {
          if (r.type === "INCOME") revenue += amt; else expense += amt;
        } else if (d >= prevKey) {
          if (r.type === "INCOME") revenuePrev += amt; else expensePrev += amt;
        }
      }
      ...
      let i = 0;
      for (const v of dayMap.values()) { days[i].count = v; i++; }
  ...
  return (
    <div className="space-y-6 lg:space-y-8 pb-12">
      ...
      <section className="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        ...
        />
        <Kpi
          icon={Receipt} label="Ticket médio"
        ...
        />
        <Kpi
          icon={AlertCircle} label="Clientes p/ retorno"
      ...
      <section className="grid gap-4 lg:gap-6 lg:grid-cols-3">
            ...
            {loading ? <Skeleton className="h-full w-full" /> : (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data?.months ?? []} margin={{ top: 8, right: 8, left: -16, bottom: 0 }}>
                  ...
                  />
                  <Area type="monotone" dataKey="receita" stroke="var(--color-primary)" strokeWidth={2.5} fill="url(#vgIncomeFill)" />
                </AreaChart>
              </ResponsiveContainer>
            )}
          </div>
        ...
        <Card className="p-5 shadow-soft">
          ...
          <div className="h-64">
            {loading ? <Skeleton className="h-full w-full" /> : (data?.topServices.length ?? 0) === 0 ? (
            ...
            <ul className="mt-3 space-y-1.5">
              {data!.topServices.map((s, idx) => (
                <li key={s.name} className="flex items-center gap-2 text-xs">
                  <span className="h-2.5 w-2.5 rounded-full shrink-0" style={{ background: PIE[idx % PIE.length] }} />
...
function Kpi({
  icon: Icon, label, value, delta, tone = "default", accent,
}: {
  icon: any;
  label: string;
  value: string | null;
