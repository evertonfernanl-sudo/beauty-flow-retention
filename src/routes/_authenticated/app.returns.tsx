...
export function RecoveryPage({ onSelectPendingCount }: { onSelectPendingCount?: () => void } = {}) {
    ...
    queryFn: async () => {
      ...
      return (
        data ?? {
          ...
          lost_count: 0,
          potential_revenue: 0,
          recovered_count_month: 0,
          recovered_value_month: 0,
          recovery_rate: 0,
          avg_days_to_recover: 0,
          avg_recovered_ticket: 0,
        }
      );
    },
  });
  ...
  const list = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        ...
        .select(
          "id, status, classification, score, days_late, potential_value, expected_return_date, last_contact_at, clients(id, name, phone, last_visit), services(name)",
        )
        .eq("company_id", companyId!)
      ...
      if (filter === "today")
        q = q.eq("expected_return_date", new Date().toISOString().slice(0, 10));
      if (filter === "week") {
        const in7 = new Date();
  ...
  const filtered = useMemo(() => {
    let rows = list.data ?? [];
    if (filter === "vip" && vipSet.data)
      rows = rows.filter((r: any) => vipSet.data!.has(r.clients?.id));
    if (search.trim()) {
    ...
    return [...rows].sort(
      (a: any, b: any) =>
        (Number(b.potential_value) * b.score) / 100 +
        b.days_late * 10 -
    ...
    <div className="space-y-6 pb-24">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Sua central de recuperação de faturamento.
          </p>
        </div>
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <Kpi
          ...
          value={formatBRL(Number(d?.potential_revenue ?? 0))}
          highlight
        />
        <Kpi
          loading={dash.isLoading}
        ...
        <Kpi
          ...
          value={`${Number(d?.recovery_rate ?? 0).toFixed(0)}%`}
        />
      </section>
      ...
      <Card className="p-5 border-primary/30 bg-gradient-to-br from-secondary/40 to-card flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs font-medium uppercase tracking-wider text-primary">
            Receita Recuperada · Este Mês
          </p>
          <p className="mt-1 text-3xl font-semibold">
            {formatBRL(Number(d?.recovered_value_month ?? 0))}
          </p>
          <p className="text-sm text-muted-foreground mt-1">
            Você recuperou {d?.recovered_count_month ?? 0} cliente
            {(d?.recovered_count_month ?? 0) === 1 ? "" : "s"} este mês 🎉
          </p>
        </div>
        <div className="text-right text-xs text-muted-foreground">
          <p>
            ...
            <span className="font-semibold text-foreground">
              {formatBRL(Number(d?.avg_recovered_ticket ?? 0))}
            </span>
          </p>
          <p>
            Tempo médio para retorno:{" "}
            <span className="font-semibold text-foreground">
              {Math.round(Number(d?.avg_days_to_recover ?? 0))}d
            </span>
          </p>
        </div>
      </Card>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        ...
        <span
          className={`grid h-8 w-8 place-items-center rounded-lg ${highlight ? "gradient-primary text-primary-foreground" : "bg-secondary text-primary"}`}
        >
...
function RecoveryRow({ row, isVip, onOpen }: { row: any; isVip: boolean; onOpen: () => void }) {
  ...
  return (
    <li
      className="py-3 px-1 sm:px-2 flex flex-wrap items-center justify-between gap-3 hover:bg-muted/30 rounded-lg transition cursor-pointer"
      onClick={onOpen}
    >
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs text-muted-foreground truncate mt-0.5">
          {row.services?.name ?? "—"} · esperado em{" "}
          {new Date(row.expected_return_date).toLocaleDateString("pt-BR")}
          {row.clients?.last_visit &&
            ` · última: ${new Date(row.clients.last_visit).toLocaleDateString("pt-BR")}`}
        </p>
      </div>
      <div className="flex items-center gap-3">
        ...
        <div className="text-right">
          <p className="text-sm font-semibold text-primary tabular-nums">
            {formatBRL(Number(row.potential_value))}
          </p>
...
function ScoreRing({ score }: { score: number }) {
  ...
  const color =
    s >= 70 ? "text-success" : s >= 40 ? "text-[color:var(--color-warning)]" : "text-destructive";
  return (
...
function ClassBadge({ cls, daysLate }: { cls: string; daysLate: number }) {
  ...
  return (
    <span className={`text-[11px] font-medium rounded-full px-2 py-0.5 border ${v.cn}`}>
      {v.label}
    </span>
  );
}
...
function RecoverySheet({
  ...
  function sendWhatsApp() {
    ...
    const tpl =
      whatsappTemplate ?? "Olá {{nome}}! Saudades de você por aqui 💕 Vamos agendar seu retorno?";
    ...
    supabase
      ...
      .eq("id", opportunityId)
      .then(() => qc.invalidateQueries({ queryKey: ["recovery"] }));
  }
      ...
      <SheetContent className="w-full sm:max-w-xl overflow-y-auto">
          ...
          <div className="mt-4 space-y-5">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="text-right">
                ...
                <p className="text-lg font-semibold text-primary">
                  {formatBRL(Number(opp.data.potential_value))}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <Stat
                label="Ticket médio"
                value={formatBRL(
                  c?.appointments_count ? Number(c.total_spent) / c.appointments_count : 0,
                )}
              />
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <Button variant="outline" onClick={() => setShowSchedule(true)} className="gap-1">
                <Calendar className="h-4 w-4" /> Agendar
              </Button>
              <Button variant="outline" onClick={() => setShowContact(true)} className="gap-1">
                <Phone className="h-4 w-4" /> Registrar
              </Button>
            </div>
                ...
                <ul className="space-y-1.5 text-sm">
                  {recent.data.map((a: any) => (
                    <li key={a.id} className="flex justify-between border-b last:border-0 py-1.5">
                      ...
                      <span className="text-muted-foreground">
                        {formatBRL(Number(a.price))} · {a.status}
                      </span>
                    </li>
                  ))}
                </ul>
        ...
        {showContact && opp.data && (
          <ContactDialog
            ...
            onSaved={() =>
              qc.invalidateQueries({ queryKey: ["client_contacts", opp.data!.client_id] })
            }
          />
        )}
...
function ContactDialog({
  ...
  const [channel, setChannel] = useState<
    "WHATSAPP" | "PHONE" | "INSTAGRAM" | "IN_PERSON" | "EMAIL"
  >("WHATSAPP");
  const [result, setResult] = useState<"ANSWERED" | "NO_ANSWER" | "SCHEDULED" | "REFUSED">(
    "NO_ANSWER",
  );
    ...
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button onClick={() => m.mutate()} disabled={m.isPending || !serviceId}>
            Criar
          </Button>
        </DialogFooter>
      </DialogContent>
