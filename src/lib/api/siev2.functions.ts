...
export const registerImportV2 = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      const { runImportParseV2 } = await import("@/lib/api/worker-v2.server");

      // Insere o job diretamente como RUNNING com tipo "import.parse_v2" para controle assíncrono
      const { data: job, error: qErr } = await supabaseAdmin
        .from("jobs")
        .insert({
          company_id: profile.company_id,
          type: "import.parse_v2",
        ...
        .select()
        .single();
      if (qErr) throw new Error(`Falha ao registrar job de processamento V2: ${qErr.message}`);

      try {
        const result = await runImportParseV2(supabaseAdmin, {
          payload: { import_id: imp.id },
          company_id: profile.company_id
        });
      ...
      console.error("[registerImportV2 SERVER ERROR]:", err);
      ...
      if (importId) {
        try {
          ...
          console.error("Erro ao registrar falha de auditoria V2 no Supabase:", dbErr);
        }
      }
...
export const applyImportRowV2 = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i) =>
  ...
  .handler(async ({ data, context }) => {
    ...
    // Enfilera na fila existente que é lida pelo worker padrão
    ...
    // Dispara o worker padrão para aplicar
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
...
export const applyImportBatchV2 = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i) =>
    z
      .object({
...
export const convertPdfToCsvV2 = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i) =>
    z
      .object({
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      console.error("[convertPdfToCsvV2 ERROR]:", err);
      return { success: false, error: err.message || String(err) };
    }
  });
